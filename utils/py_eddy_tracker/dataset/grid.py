...
class GridDataset(object):
    ...
    def setup_coordinates(self):
        ...
        if self.is_centered:
            ...
            # self.vars entries load as MaskedArray even for coordinate axes,
            # which are never actually missing data; numba can't compile
            # against MaskedArray, so store the plain values.
            self.x_c = ma.getdata(self.vars[x_name].astype("float64"))
            self.y_c = ma.getdata(self.vars[y_name].astype("float64"))
            ...
            # Same MaskedArray-to-plain fix as the is_centered branch above.
            self.x_bounds = ma.getdata(self.vars[x_name].astype("float64"))
            self.y_bounds = ma.getdata(self.vars[y_name].astype("float64"))
        ...
        for anticyclonic_search in [True, False]:
            ...
            for coll_ind, coll in enumerate(self.contours.iter(step=iterator)):
                ...
                for contour in contour_paths:
                    ...
                    # Pass the plain values (masked-pixel case already handled
                    # above); numba can't compile against MaskedArray.
                    if has_value(
