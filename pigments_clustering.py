...
# Primary PFT per diagnostic pigment from the literature, not derived from this dataset.
PIGMENT_TO_PFT = {
    "Fuco": "Diatoms",
    "chl c1+c2": "Diatoms",
    "HexFuco": "Haptophytes",
...
def build_cluster_pft_label(pigments_in_cluster: list[str]) -> str:
    """Build a ' + '-joined PFT label for a cluster, ordered by the bio-optics convention."""
    pft_order = [
...
def get_pigment_cols(df: pd.DataFrame) -> list[str]:
    ...
    # T chla is the sum across all chlorophyll-a forms and trivially correlates with everything; exclude it so biomarker groupings stand out.
    exclude_cols = {"T chla"}
...
def compute_per_eddy_mean_correlation(
    ...
    Eddies with fewer than min_pixels clean pixels, or whose correlation matrix contains any NaN (e.g. a pigment is constant within the eddy), are skipped.
    """
    ...
    # list of (n_pigments, n_pigments) -> (n_eddies, n_pigments, n_pigments) -> (n_pigments, n_pigments)
...
def compute_linkage_from_correlation(corr: np.ndarray) -> np.ndarray:
    ...
    dist = 1 - corr
    # squareform with checks=False takes the upper triangle only, so this average is what removes the floating-point asymmetry that pandas corr() leaves in the lower triangle.
    dist = (dist + dist.T) / 2
    np.fill_diagonal(dist, 0.0)
    # (n_pigments, n_pigments) -> condensed (n_pigments * (n_pigments - 1) / 2,)
...
def save_dendrogram(
    ...
    for cid, xs in cluster_tick_xs.items():
        ...
        ax.text(
            mid, y_anno, label, ha="center", va="bottom",
...
def main() -> None:
    ...
    if args.per_eddy:
        ...
        suffix = "_per_eddy"
        title = f"Hierarchical Clustering of Diagnostic Pigments (per-eddy, avg over {n_eddies} eddies)"
        np.save(
            resolve_output_dir(args.experiment, "pigments_clustering") / f"mean_correlation{suffix}.npy",
            mean_corr,
        )
    else:
    ...
    save_dendrogram(
        Z, pigment_cols, out_dir / f"dendrogram{suffix}.png", args.threshold, title
    )
    save_cluster_table(
        Z, pigment_cols, args.threshold, out_dir / f"cluster_assignments{suffix}.csv"
    )
