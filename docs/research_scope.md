...
- **Open-ocean mask:** Before eddy identification, require finite absolute dynamic topography (`adt`), eastward velocity (`ugos`), northward velocity (`vgos`), and `relative_vorticity`. Keep cells at least eight grid cells from invalid data. Exclude the Great Lakes rectangle from 81°W to 75°W and 40°N to 44°N. The eight-cell distance is approximately 1° on this grid. The bronze file stores the mask as the `open_ocean` label, 1 for a kept cell, and keeps each unmasked field as `adt_full`, `ugos_full`, `vgos_full`, and `relative_vorticity_full`. Every analysis stage reads the masked names. Read a `_full` field only for a figure: the unmasked velocities hold spurious fast cells against the land.
- **Gulf Stream axis:** The daily axis is a contour of the unmasked absolute dynamic topography (`adt_full`), and geostrophic speed picks its level. For each day, take the longest contour at every level in `gulf_stream.adt_level_range` (0.10 to 0.60 m in 0.025 m steps for the Gulf Stream), keep the ones that reach the eastern edge of the region, and score each by the mean speed (`ugos_full`, `vgos_full`) along it. Smooth the best level with a 15-day centered rolling median, then draw that day's contour at the smoothed level and resample it every 5 km, ordered from west to east. The level follows the seasonal rise of the sea surface, from about 0.275 m in winter to 0.425 m in early autumn, so a fixed level would leave the core for part of the year. A contour is an ordered curve, not one latitude per longitude: a meander folds back, the jet flows north below Cape Hatteras, and a ring is a separate closed contour that the axis cannot fall into. An eddy or a pixel is on side `N` when it lies to the left of the flow at the nearest axis point and on side `S` when it lies to the right; this equals north and south wherever the jet flows east. The mean axis is the same procedure run on the time-mean fields (`trace_mean_streamline`). The distance from a pixel to the axis is the signed cross-stream distance to that day's curve (`compute_signed_distance_grid_km`), and a pixel whose nearest axis point is an end of the line has no distance. Adopted 2026-09-21.
...
| Collocation | The stage selects the speed contour near the composite midpoint, applies an 80% interior coverage threshold, and keeps every valid pixel within `max_radius` speed radii of the center or inside the contour, with an `inside_contour` flag per pixel. | It does not measure annular coverage. |
